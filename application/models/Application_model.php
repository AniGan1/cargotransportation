...
class Application_model extends CI_Model
{
    ...
    public function drivers(){
        $sql = 'select * from travel_list join drivers on travel_list.id_driver=drivers.id_driver';
        $result = $this->db->query($sql);
        return $result->result_array();
    }
    ...
    public function insert_put_list($number_list, $id_application, $transported, $dispatch_date, $the_factual_delivery_date, $id_car, $id_driver){
        $sql = 'insert into travel_list (number_list, id_application, transported, dispatch_date, the_factual_delivery_date, status_list, id_car, id_driver) values (?,?,?,?,?,"В пути",?,?)';
        $result = $this->db->query($sql, [$number_list, $id_application, $transported, $dispatch_date, $the_factual_delivery_date, $id_car, $id_driver]);
    }
    
    public function otcte_applis($date_start=False, $date_end=false){
        if ($date_start !=False && $date_end != false) {
            ...
            where contracts.conclusion_date >= "'.$date_start.'" and contracts.due_date <= "'.$date_end.'"';        
        }else{
            $sql = 'select * from contracts
            join applications on contracts.id_application = applications.id_application
            join clients on applications.id_client = clients.id_client';
    ...
    public function transp($date_start=False, $date_end=false, $fio=False){
        if ($date_start !=False && $date_end != false && $fio != false) {
            $sql = 'select * from travel_list
            join applications on travel_list.id_application = applications.id_application
            join cars on travel_list.id_car = cars.id_car
            join drivers on travel_list.id_driver = drivers.id_driver
            join contracts on applications.id_application = contracts.id_application
            where contracts.conclusion_date >= "'.$date_start.'" and contracts.due_date <= "'.$date_end.'" and drivers.id_driver = "'.$fio.'"';   
        }else{
            $sql = 'select * from travel_list
            join applications on travel_list.id_application = applications.id_application
            join cars on travel_list.id_car = cars.id_car
        ...
        $result = $this->db->query($sql, [$date_start, $date_end, $fio]);
        return $result->result_array();
    }
}
